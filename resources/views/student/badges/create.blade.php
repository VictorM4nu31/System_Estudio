<x-layouts.app :title="__('Crear Insignia - Estudiante')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Crear Insignia</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Crea una nueva insignia para estudiantes</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('student.badges.store') }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="type" 
                                ...
                                <option value="misiones" {{ old('type') == 'misiones' ? 'selected' : '' }}>Misiones</option>
                                <option value="tareas" {{ old('type') == 'tareas' ? 'selected' : '' }}>Tareas</option>
                                <option value="gremios" {{ old('type') == 'gremios' ? 'selected' : '' }}>Gremios</option>
                                <option value="recompensas" {{ old('type') == 'recompensas' ? 'selected' : '' }}>Recompensas</option>
                                <option value="actividades" {{ old('type') == 'actividades' ? 'selected' : '' }}>Actividades</option>
                                <option value="especial" {{ old('type') == 'especial' ? 'selected' : '' }}>Especial</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea id="criteria" 
                                  ...
                                  class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('criteria') }}</textarea>
                        @error('criteria')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
