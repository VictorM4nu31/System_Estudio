<x-layouts.app :title="__('Crear Datos Académicos - Estudiante')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Crear Datos Académicos</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Registra tu información académica</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('student.academic-data.store') }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <select id="semester" 
                                ...
                                <option value="1" {{ old('semester') == '1' ? 'selected' : '' }}>Primer Semestre</option>
                                <option value="2" {{ old('semester') == '2' ? 'selected' : '' }}>Segundo Semestre</option>
                                <option value="3" {{ old('semester') == '3' ? 'selected' : '' }}>Tercer Semestre</option>
                                <option value="4" {{ old('semester') == '4' ? 'selected' : '' }}>Cuarto Semestre</option>
                                <option value="5" {{ old('semester') == '5' ? 'selected' : '' }}>Quinto Semestre</option>
                                <option value="6" {{ old('semester') == '6' ? 'selected' : '' }}>Sexto Semestre</option>
                            </select>
                            @error('semester')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea id="notes" 
                                  ...
                                  class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
