<x-layouts.app :title="__('Editar Actividad - Estudiante')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Actividad</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Modifica la información de la actividad</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('student.activities.update', $activity) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="type" 
                                ...
                                <option value="taller" {{ old('type', $activity->type) == 'taller' ? 'selected' : '' }}>Taller</option>
                                <option value="conferencia" {{ old('type', $activity->type) == 'conferencia' ? 'selected' : '' }}>Conferencia</option>
                                <option value="seminario" {{ old('type', $activity->type) == 'seminario' ? 'selected' : '' }}>Seminario</option>
                                <option value="competencia" {{ old('type', $activity->type) == 'competencia' ? 'selected' : '' }}>Competencia</option>
                                <option value="exposicion" {{ old('type', $activity->type) == 'exposicion' ? 'selected' : '' }}>Exposición</option>
                                <option value="workshop" {{ old('type', $activity->type) == 'workshop' ? 'selected' : '' }}>Workshop</option>
                                <option value="otro" {{ old('type', $activity->type) == 'otro' ? 'selected' : '' }}>Otro</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <select id="guild_id" 
                                ...
                                <option value="1" {{ old('guild_id', $activity->guild_id) == '1' ? 'selected' : '' }}>Gremio de Programación</option>
                                <option value="2" {{ old('guild_id', $activity->guild_id) == '2' ? 'selected' : '' }}>Gremio de Diseño</option>
                                <option value="3" {{ old('guild_id', $activity->guild_id) == '3' ? 'selected' : '' }}>Gremio de Marketing</option>
                            </select>
                            @error('guild_id')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" 
                                   id="duration" 
                                   name="duration" 
                                   value="{{ old('duration', $activity->duration ?? 2) }}"
                                   min="0.5" 
                                   max="24"
                                   step="0.5"
                                   class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   ...
                                   value="{{ old('max_participants', $activity->max_participants ?? 20) }}"
                                   min="1" 
                                   max="100"
                                   class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <textarea id="requirements" 
                                  ...
                                  class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('requirements', $activity->requirements) }}</textarea>
                        @error('requirements')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" 
                               id="points_reward" 
                               name="points_reward" 
                               value="{{ old('points_reward', $activity->points_reward ?? 50) }}"
                               min="0" 
                               max="1000"
                               class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                        @error('points_reward')
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" 
                                   name="participated" 
                                   value="1"
                                   {{ old('participated', $activity->participated) ? 'checked' : '' }}
