<x-layouts.app :title="__('Detalle de la Misión - Docente')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-3">
                <a href="{{ route('teacher.missions.edit', $mission) }}"
                ...
                <a href="{{ route('teacher.missions.index') }}"
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="p-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4">Información de la Misión</h2>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <!-- Basic Info -->
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Título de la Misión</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white font-medium">{{ $mission->title }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">ID de la Misión</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $mission->id }}</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Gremio Asignado</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $mission->guild->name ?? 'No asignado' }}</p>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-1 p-3 bg-gray-50 dark:bg-gray-700 rounded-md">
                                <p class="text-sm text-gray-900 dark:text-white">
                                    {{ $mission->description ?: 'No hay descripción disponible.' }}
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div class="mt-6 pt-6 border-t border-gray-200 dark:border-gray-700">
                    ...
                    <div class="flex flex-wrap gap-3">
                        <a href="#" class="inline-flex items-center px-3 py-2 text-sm font-medium text-blue-700 bg-blue-50 border border-blue-200 rounded-md hover:bg-blue-100">
                            <flux:icon.clipboard-document-list class="size-4 mr-2" />
                            Crear Tarea
                        </a>
