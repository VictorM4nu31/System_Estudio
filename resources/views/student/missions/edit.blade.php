<x-layouts.app :title="__('Editar Misión - Estudiante')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Misión</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Modifica la información de la misión</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('student.missions.update', $mission) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="difficulty" 
                                ...
                                <option value="facil" {{ old('difficulty', $mission->difficulty) == 'facil' ? 'selected' : '' }}>Fácil</option>
                                <option value="medio" {{ old('difficulty', $mission->difficulty) == 'medio' ? 'selected' : '' }}>Medio</option>
                                <option value="dificil" {{ old('difficulty', $mission->difficulty) == 'dificil' ? 'selected' : '' }}>Difícil</option>
                                <option value="experto" {{ old('difficulty', $mission->difficulty) == 'experto' ? 'selected' : '' }}>Experto</option>
                            </select>
                            @error('difficulty')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="datetime-local" 
                                   id="due_date" 
                                   name="due_date" 
                                   value="{{ old('due_date', $mission->due_date ? $mission->due_date->format('Y-m-d\TH:i') : '') }}"
                                   class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                            @error('due_date')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   ...
                                   value="{{ old('max_participants', $mission->max_participants ?? 1) }}"
                                   min="1" 
                                   max="10"
                                   class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">
                    ...
                    <div>
                        ...
                        <textarea id="requirements" 
                                  ...
                                  class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('requirements', $mission->requirements) }}</textarea>
                        @error('requirements')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        <label class="flex items-center">
                            <input type="checkbox" 
                                   ...
                                   value="1"
                                   {{ old('is_team_mission', $mission->is_team_mission) ? 'checked' : '' }}
