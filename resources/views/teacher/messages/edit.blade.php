<x-layouts.app :title="__('Editar Mensaje - Docente')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Editar Mensaje</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Modifica el contenido del mensaje</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700">
            <div class="p-6">
                <form method="POST" action="{{ route('teacher.messages.update', $message->id) }}" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="recipient_type" id="recipient_type" onchange="toggleRecipientFields()" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white" required>
                            ...
                            <option value="guild" {{ old('recipient_type', $message->guild_id ? 'guild' : 'student') == 'guild' ? 'selected' : '' }}>Gremio</option>
                            <option value="student" {{ old('recipient_type', $message->guild_id ? 'guild' : 'student') == 'student' ? 'selected' : '' }}>Estudiante Específico</option>
                        </select>
                        @error('recipient_type') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>

                    <!-- Gremio (se muestra si se selecciona gremio) -->
                    <div id="guild_field" class="{{ old('recipient_type', $message->guild_id ? 'guild' : 'student') == 'guild' ? '' : 'hidden' }}">
                        ...
                        <select name="guild_id" id="guild_id" class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                            ...
                            @foreach($guilds ?? [] as $guild)
                                <option value="{{ $guild->id }}" {{ old('guild_id', $message->guild_id) == $guild->id ? 'selected' : '' }}>
                                    {{ $guild->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div id="student_field" class="{{ old('recipient_type', $message->guild_id ? 'guild' : 'student') == 'student' ? '' : 'hidden' }}">
                    ...
                    <div class="flex items-center justify-end space-x-3 pt-6 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <button type="submit" class="px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            <flux:icon.check class="size-4 mr-2 inline" /> Actualizar Mensaje
                        </button>
                    </div>
                </form>
            </div>
