<x-layouts.app :title="__('Detalles de la Misión - Estudiante')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">{{ $mission->title }}</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Detalles de la misión</p>
            </div>
            <div class="flex space-x-3">
                <a href="{{ route('student.missions.edit', $mission) }}" class="inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-blue-600 border border-transparent rounded-md shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-2">
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                    <h2 class="text-lg font-medium text-gray-900 dark:text-white mb-4">Información de la Misión</h2>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Descripción</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $mission->description }}</p>
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Dificultad</label>
                                @if($mission->difficulty == 'facil')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200">
                                        {{ ucfirst($mission->difficulty) }}
                                    </span>
                                @elseif($mission->difficulty == 'medio')
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-200">
                                        {{ ucfirst($mission->difficulty) }}
                                    </span>
                                @elseif($mission->difficulty == 'dificil')
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Puntos</label>
                                <p class="mt-1 text-sm font-semibold text-gray-900 dark:text-white">{{ $mission->points }}</p>
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $mission->requirements }}</p>
                        </div>
                        @endif
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-900 dark:text-white">
                                    {{ $mission->due_date ? $mission->due_date->format('d/m/Y H:i') : 'Sin fecha límite' }}
                                </p>
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Máximo de Participantes</label>
                                <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $mission->max_participants ?? 'Sin límite' }}</p>
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Tipo de Misión</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">
                                {{ $mission->is_team_mission ? 'Misión de Equipo' : 'Misión Individual' }}
                            </p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Fecha de Creación</label>
                            <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $mission->created_at->format('d/m/Y H:i') }}</p>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                    ...
                    <div class="space-y-4">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="text-sm font-semibold text-gray-900 dark:text-white">8/{{ $mission->max_participants ?? '∞' }}</span>
                        </div>
                        
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">Completada</span>
                            <span class="text-sm font-semibold text-gray-900 dark:text-white">3</span>
                        </div>
                        
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">En Progreso</span>
                        ...
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <span class="text-sm text-gray-900 dark:text-white">75%</span>
                                </div>
                            </td>
