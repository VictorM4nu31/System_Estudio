<x-layouts.app :title="__('Crear Recompensa - Estudiante')">
    <div class="flex h-full w-full flex-1 flex-col gap-6">
        <!-- Header -->
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Crear Recompensa</h1>
                <p class="text-sm text-gray-600 dark:text-gray-400">Crea una nueva recompensa para estudiantes</p>
            </div>
        ...
        <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
            <form action="{{ route('student.rewards.store') }}" method="POST">
                @csrf
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <select id="type" 
                                ...
                                <option value="puntos" {{ old('type') == 'puntos' ? 'selected' : '' }}>Puntos</option>
                                <option value="insignia" {{ old('type') == 'insignia' ? 'selected' : '' }}>Insignia</option>
                                <option value="nivel" {{ old('type') == 'nivel' ? 'selected' : '' }}>Nivel</option>
                                <option value="item" {{ old('type') == 'item' ? 'selected' : '' }}>Item</option>
                                <option value="privilegio" {{ old('type') == 'privilegio' ? 'selected' : '' }}>Privilegio</option>
                            </select>
                            @error('type')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <textarea id="conditions" 
                                  ...
                                  class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-white shadow-sm focus:border-blue-500 focus:ring-blue-500 sm:text-sm">{{ old('conditions') }}</textarea>
                        @error('conditions')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
